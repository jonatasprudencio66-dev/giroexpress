    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl p-8 space-y-6 shadow-2xl">
        ...
        <form onSubmit={submit} className="space-y-4" data-testid="register-form">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-slate-400 block mb-1 font-semibold">
                # {role === "store" ? "Nome da Loja" : "Seu nome"}
              </label>
            ...
            {role === "store" && (
              <div className="md:col-span-2">
                <label className="text-xs text-slate-400 block mb-1 font-semibold">
                  # Endereço da Loja (para retirada padrão)
                </label>
          ...
          {err && (
            <div data-testid="reg-error" className="bg-rose-500/10 border border-rose-500/30 text-rose-400 text-sm p-3 rounded-xl">
              # {err}
            </div>
          )}
          ...
          <p className="text-sm text-slate-400 text-center">
            # Já tem conta?{" "}
            <Link to="/login" className="text-orange-400 font-bold hover:underline" data-testid="link-login">
              # Fazer login
            </Link>
          </p>
        </form>
